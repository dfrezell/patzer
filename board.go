...
func (p *Piece)pawnMove(pos int8) []int8 {
	var endPos []int8
	if p.Color == WHITE {
		endPos = whitePawnMove()
	} else if p.Color == BLACK {
		endPos = blackPawnMove()
	} else {
		endPos = []int8{}
	}
...
type Board struct {
	...
	Pieces [PIECES_MAX][COLOR_MAX][]int8
...
func NewBoard() *Board {
	...
	b.Pieces[PAWN][WHITE] = []int8{A2,B2,C2,D2,E2,F2,G2,H2}
    b.Pieces[PAWN][BLACK] = []int8{A7,B7,C7,D7,E7,F7,G7,H7}
    b.Pieces[KNIGHT][WHITE] = []int8{B1,G1}
    b.Pieces[KNIGHT][BLACK] = []int8{B8,G8}
    b.Pieces[BISHOP][WHITE] = []int8{C1,F1}
    b.Pieces[BISHOP][BLACK] = []int8{C8,F8}
    b.Pieces[ROOK][WHITE] = []int8{A1,H1}
    b.Pieces[ROOK][BLACK] = []int8{A8,H8}
    b.Pieces[QUEEN][WHITE] = []int8{D1}
    b.Pieces[QUEEN][BLACK] = []int8{D8}
    b.Pieces[KING][WHITE] = []int8{E1}
    b.Pieces[KING][BLACK] = []int8{E8}

	for p, _ := range b.Pieces {
		for c, _ := range b.Pieces[p] {
			for _, x := range b.Pieces[p][c] {
